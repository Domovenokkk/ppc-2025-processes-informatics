...
void FindGlobalMinMax(const std::vector<uint8_t> &local_pixels, uint8_t &local_min, uint8_t &local_max,
  ...
  local_min = MAX_PIXEL_VALUE;
  local_max = MIN_PIXEL_VALUE;
...
std::vector<uint8_t> ProcessLocalPixels(const std::vector<uint8_t> &local_pixels, uint8_t global_min,
  ...
  double scale = static_cast<double>(MAX_PIXEL_VALUE - MIN_PIXEL_VALUE) / static_cast<double>(global_max - global_min);
...
void GatherResults(int rank, int size, const std::vector<uint8_t> &local_result, int width, int height,
  ...
  MPI_Gatherv(local_result.data(), local_size, MPI_UINT8_T, final_output.data() + 2, recv_counts.data(), displs.data(),
  ...
  MPI_Bcast(final_output.data(), total_output_size, MPI_UINT8_T, 0, MPI_COMM_WORLD);
}
...
bool ContrastEnhancementMPI::RunImpl() {
  ...
  try {
    ...
    uint8_t local_min, local_max, global_min, global_max;
